...
class DisallowDefaultContextCreation implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (implode('\\', $node->class->getParts()) !== 'HeyFrame\\Core\\Framework\\Context') {
            return [];
        }

        $class = $this->reflectionProvider->getClass('HeyFrame\Core\Framework\Context');
        ...
        return [
            RuleErrorBuilder::message(sprintf('Do not use %s::createDefaultContext() function in code.', $node->class->toString()))
                ...
                ->identifier('heyframe.disallow.default.context.creation')
                ->build(),
        ];
    }
}
