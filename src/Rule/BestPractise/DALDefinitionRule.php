...
namespace HeyFrame\PhpStan\Rule\BestPractise;
...
use HeyFrame\PhpStan\Collector\DALDefinitionCollector;
use HeyFrame\PhpStan\Collector\DALEntityCollector;
        ...
        foreach ($definitions as $definition) {
            ...
            foreach ($definition['fields'] as $fieldName => $definitionField) {
                if (!isset($entity['properties'][$fieldName])) {
                    ...
                    ->identifier('heyframe.bestPractise.dal.propertyMissing')
                        ->line(1)
                        ->file($entity['file'])
                        ->build();
                ...
                if ($field['readonly'] === true) {
                    $errors[] = RuleErrorBuilder::message(
                        ...
                        ->identifier('heyframe.bestPractise.dal.propertyReadonly')
                        ->line((int) $field['line'])
                        ->file($entity['file'])
                        ->build();
                }
                ...
                if ($field['visibility'] === 'private') {
                    $errors[] = RuleErrorBuilder::message(
                        ...
                        ->identifier('heyframe.bestPractise.dal.propertyPrivate')
                        ->line((int) $field['line'])
                        ->file($entity['file'])
                        ->build();
                }
                ...
                if ($field['visibility'] === 'protected') {
                    ...
                    if (!$getterExists) {
                        $errors[] = RuleErrorBuilder::message(
                            ...
                            ->identifier('heyframe.bestPractise.dal.noGetter')
                            ->line((int) $field['line'])
                            ->file($entity['file'])
                            ->build();
                    }
                    ...
                    if (!$definitionField['runtime'] && !$definitionField['computed']) {
                        if (!isset($entity['methods']['set' . ucfirst($fieldName)])) {
                            $errors[] = RuleErrorBuilder::message(
                                ...
                                ->identifier('heyframe.bestPractise.dal.noSetter')
                                ->line((int) $field['line'])
                                ->file($entity['file'])
                                ->build();
                        }
